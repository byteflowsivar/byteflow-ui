...
export const ResizablePanelGroup: React.FC<ResizablePanelGroupProps> = ({
    ...
    return (
        <div 
            className={`bf-resizable-group bf-resizable-group--${direction} ${className}`}
            {...props}
        >
            {children}
        </div>
...
export const ResizablePanel: React.FC<ResizablePanelProps> = ({
    ...
    const style = defaultSize ? { flex: `${defaultSize} 1 0%`, ...styleProp } : { flex: '1 1 0%', ...styleProp };
    return (
        <div 
            className={`bf-resizable-panel ${className}`} 
            style={style}
            {...props}
        >
            {children}
...
export const ResizableHandle: React.FC<ResizableHandleProps> = ({
    ...
    return (
        <div 
            className={`bf-resizable-handle ${className}`}
            {...props}
        >
            {withHandle && (
