...
/**
 * Propiedades del componente Pagination (Paginación).
 */
export interface PaginationProps extends Omit<React.HTMLAttributes<HTMLElement>, 'onChange'> {
    /** Número total de elementos a paginar. */
    total: number;
    /** Página actual (empezando por 1). */
    current: number;
    /** Callback disparado cuando se cambia la página. */
    onChange: (page: number) => void;
    /** Número de elementos por página. Por defecto 10. */
    pageSize?: number;
    /** Número de páginas adyacentes a mostrar alrededor de la actual. */
    siblingCount?: number;
    /** Si se deben mostrar los botones de ir al inicio/final (opcional). */
    showEdges?: boolean;
}

/**
 * Pagination: Componente para navegar entre múltiples páginas de contenido.
 */
export const Pagination = React.forwardRef<HTMLElement, PaginationProps>(
    ({
        ...
        return (
            <nav
                ref={ref}
                className={`bf-pagination ${className}`}
